...
int CreateFile(char* address){
    char* check;
    check=(char*)malloc(NAME_SIZE*sizeof(char));
...
void Undo(char* address){
    ...
    strcat(address2, address);
    // printf("%s", address2);
    CopyFile(address,"komaki.txt", 0, -1);
    ClearFile(address);
    CopyFile(address2, address, 0, -1);
    ClearFile(address2);
...
int main() {  
    ...
    // Remove("root/me/you/v2.txt", 1, 7, 7, 'f');
    ...
    // printf("%d", l);
    // Compare("root/v1.txt","root/test.txt");
    // Tree("C:/proj", 0, -2);
    // ................................................
        ...
        if (strcmp(command, "insertstr")==0){
            ...
            // printf("%s %s\n", address ,str);
                ...
                if (invalid!=2){
                    ...
                    invalid = sscanf(str+1, "%[^\"]%*c --pos %d:%d", str, &line, &pos);
                    if (invalid!=3){
                        print_invalid_input();
                    }                     
                    else{
                        // printf("%s %s %d %d %d\n", address1 , str, line, pos, invalid);
                        printf("%s %s %d %d \n", address1 , str, line, pos);
                        strcat(address2, address1);
                        // strcat(str, '\0');
                        SaveChangeFile(address1);
                        insertstr(address2, str, line, pos);
                        
                    }
                }
                else{
                    invalid = sscanf(str, "%s --pos %d:%d", str, &line, &pos);
                    printf("%s %d %d", str, line, pos);
                    if (invalid!=3){
                        print_invalid_input();
                        printf("sasas\n");
                    } 
